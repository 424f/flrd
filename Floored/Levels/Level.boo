...
class Level:
	...
	protected def CreateBox(material as Material, center as Vector3, dim as Vector3, hasMass as bool) as GameObject:
		...
		bodyDef = BodyDef()
		bodyDef.Position = center.AsVec2() * scale
		body = World.CreateBodyWithoutMassFromShape(bodyDef, box.CreatePhysicalRepresentation(), 3.0f, 0.1f, 0.0f)
		if hasMass:
	...
	public def constructor(world as World):
		...
		Crate = CreateBox(crate, Vector3(-12.0F, 4.0F, 0), Vector3(2.0F, 2.0F, 2.0F), false)
		//Crate.IsStatic = false
		Boxes.Add(Crate)
		Crate = CreateBox(crate, Vector3(12.0F, 30.0F, 0.0F), Vector3(2.0F, 2.0F, 2.0F), false)
		//Crate.IsStatic = false
		Boxes.Add(Crate)
		Crate2 = CreateBox(crate, Vector3(18.0F, 40.0F, 0.0F), Vector3(3.0F, 3.0F, 3.0F), false)
